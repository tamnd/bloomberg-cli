// Package cli builds the bloom command tree on top of the bloomberg library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: bloomberg.DefaultConfig()}

	root := &cobra.Command{
		Use:   "bloom",
		Short: "Read Bloomberg financial news and market updates",
		Long: `bloom reads Bloomberg financial news through its public RSS feeds.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

bloom is an independent tool and is not affiliated with Bloomberg L.P.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	root.AddCommand(
		app.feedSectionCmd("top", "Top Bloomberg headlines", "top"),
		app.feedSectionCmd("markets", "Markets news", "markets"),
		app.feedSectionCmd("tech", "Technology news", "technology"),
		app.feedSectionCmd("politics", "Politics and government news", "politics"),
		app.feedSectionCmd("business", "Business and corporate news", "business"),
		app.feedSectionCmd("economics", "Economics and macro news", "economics"),
		app.feedSectionCmd("crypto", "Cryptocurrency and digital asset news", "crypto"),
		app.feedCmd(),
		app.sectionsCmd(),
		newVersionCmd(),
	)
...
func (a *App) setup() error {
	...
	a.client = bloomberg.NewClient(a.cfg)
	return nil
}
