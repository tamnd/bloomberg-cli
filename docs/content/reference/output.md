...
describes what users get. Pick a format with `-o`, or let bloom choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bloom <command> -o table   # aligned columns for reading
bloom <command> -o jsonl   # one JSON object per line, for piping
bloom <command> -o json    # a single JSON array
bloom <command> -o csv     # spreadsheet friendly
bloom <command> -o tsv     # tab-separated
bloom <command> -o url     # just the URL column
bloom <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bloom <command> --fields id,title,url
```
...
```bash
bloom <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bloom <command>            # a table, because this is a terminal
bloom <command> | wc -l    # JSONL, because this is a pipe
```
